...
const MyNetworksPage = () => {
  ...
  const followerStartIndex = (currentPageFollows - 1) * itemsPerPage;
  const followerEndIndex = currentPageFollows * itemsPerPage;
  ...
  return (
    <PostsLayout title='Connections' content='Network page'>
          ...
          follows.slice(followsStartIndex, followsEndIndex).map((follow) => (
            <div key={follow.id} class="block rounded-lg w-60 m-2 p-2 mx-auto md:mx-2 bg-[#f8eeeb] text-[#000000] shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
              <Link to={ `/profile/${follow.email}` }>
                <img class="rounded-full w-1/2 mx-auto" src={follow.profile_image} alt="Profile" style={{ borderRadius: '50%' }} />
              </Link>
              <div class="p-2">
      ...
      <h2 className="mt-8 text-[#000000]">Following:</h2>
      <div className="mt-2" style={{ display: 'flex', flexWrap: 'wrap' }}>
      {profiles ? (
          profiles.slice(profilesStartIndex, profilesEndIndex).map((follow) => (
            <div key={follow.id} class="block rounded-lg w-60 m-2 p-2 mx-auto md:mx-2 bg-[#f8eeeb] text-[#000000] shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
              ...
              <div class="p-2">
                ...
                <button
                    type="button"
                    className="inline-block ml-2 rounded-full bg-[#ff3d3d] text-white justify-start px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal"
                    data-te-ripple-init
                    data-te-ripple-color="light"
                    title="Unfollow"
        ...
        {profiles.length === 0 ? <h4 className='text-center raleway'> You are not following anyone</h4> : null}
      </div>
      <div className="pagination text-[#000000]">
        <button
      ...
      <h2 className="mt-8 text-[#000000]">Followers:</h2>
      <div className="mt-2" style={{ display: 'flex', flexWrap: 'wrap' }}>
      {followers ? (
          followers.slice(followerStartIndex, followerEndIndex).map((follow) => (
            <div key={follow.id} class="block rounded-lg w-60 m-2 p-2 mx-auto md:mx-2 bg-[#f8eeeb] text-[#000000] shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
              <Link to={ `/profile/${follow.email}` }> 
                <img class="rounded-full w-1/2 mx-auto" src={follow.profile_image} alt="Profile" />
              </Link>
